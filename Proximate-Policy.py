...
def clipped_surrogate(policy, old_probs, states, actions, rewards,
    ...
    new_states=torch.cat([i.unsqueeze(0) for i in states],dim=0)
    new_probs=policy(new_states)
    actions=torch.tensor(np.array(actions))
    rewards=torch.tensor(np.array(rewards))
    normalize_rewards= (rewards-torch.mean(rewards,dim=0))/torch.std(rewards,dim=0)
    ...
    normalized_rewards = (futher_discounted_rewards-mean[:, np.newaxis])/std[:, np.newaxis]
